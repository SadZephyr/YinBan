...
本文件固化阶段 6 页面实施蓝图。唯一功能和逻辑基线为 v1.4、当前工程实际 Kotlin/XML/ViewBinding、14 张真实运行截图。T1 是患者首页及所有登录后页面的第一视觉基准；T2、T3、T4 只作为亮度、信息层级和组件细节的辅助参考。团队样图只决定视觉语言，不决定功能数量、控件数量或页面结构。

已完成并通过的 6.2 登录三步页面暂时保留浅色品牌入口，不在 6.3—6.7 中重新实施；6.8 最终统一阶段再评估它与 T1 登录后主应用之间的品牌衔接。
...
从 6.3 开始，所有登录后页面必须继承 T1 深色沉浸体系：

- 页面背景使用深靛紫 / 深蓝紫静态渐变或多层静态背景，不使用动态粒子和高成本实时模糊。
- 半透明卡片必须承担真实信息分组功能，使用统一 night Token、轻描边和受控透明度，不得为了效果单独发明玻璃样式。
- 黑色小影火是核心品牌视觉；`开心.png` 只在品牌、主状态、AI 或空状态中克制使用，不替代功能图标。
- 顶部连接状态、主状态、设备控制、快捷入口、SOS 和底部导航应形成稳定纵向层级。
- 文字优先使用白色或柔和浅紫，避免黑底纯白大段高眩光正文。
- `#5B5CEB` 保持唯一主交互色；红色只用于 SOS、危险确认和真正高风险状态。
- 底部导航使用深色半透明容器，图标和文字完整可读。
- 页面信息密度高于此前被否决的浅色患者首页，但不得在 1080x2400 下拥挤。
- T1 样图中不存在于 v1.4 的功能不得加入，包括“主动分享”、每日一句刷新、支持者、新通知入口、新监控数据和新页面跳转。
- 真实视频区域不得被紫色滤镜、光晕、玻璃遮罩、渐变或持续动画覆盖。

登录页例外：6.2 已通过的浅色登录三步在本阶段保留；未经用户批准，不得在 6.3—6.7 中修改登录页。
...
- 以 T1 为主要参考，彻底撤销此前浅色患者首页方向。
- 页面使用深靛紫 / 深蓝紫沉浸背景，可包含静态多层渐变和少量柔和紫色光感。
- 不使用动态粒子，不使用实时高成本 blur，不引入第三方模糊依赖。
- 背景和卡片不能影响滚动、底部导航性能、文本可读性或控件触达。
- 黑色小影火是主状态区核心品牌视觉，使用 `开心.png` 时不得拉伸、裁切、改色或加高饱和发光。
- 页面信息密度可高于被否决的浅色版本，但必须保持呼吸感和明确层级。

最终页面区块顺序：

1. T1 深色页面背景：`activity_patient.xml` 与首页 Fragment 的外层背景统一使用 night 背景语义。
2. 顶部连接状态卡：`banner_connection`、`dot_status`、`tv_connection_status`、`tv_room_number`，使用深色半透明卡片、轻描边和清晰状态圆点；不得加入 T1 样图中的“主动分享”。
3. Fragment 容器：`fragment_container`，承载首页和其他 Tab。
4. 小影火主状态区：`card_standby`、`tv_standby_title`、`tv_standby_subtitle`，位于页面上半区，可使用圆形或柔和光晕容器；保留“AI 影伴待命中”“等待监护人连入”和当前真实状态文案。
5. 每日一句：`tv_daily_quote`，使用独立半透明信息卡；保留当前真实内容，不新增刷新点击功能。
6. 设备控制卡：`switch_video`、`switch_audio`，使用紧凑双行深色半透明卡片，保留摄像头、麦克风、原 ViewBinding、原 checked 监听和 `stream_start` / `stream_stop` 链路。
7. 并列快捷入口：`btn_send_location`、`btn_send_device_status`，两个入口不能合并，使用统一深色次级按钮或半透明卡片。
8. SOS：`btn_sos`，位于底部导航上方，醒目但控制红色亮度和面积。
9. 底部导航：`bottom_nav`，保留患者端四项“主页 / 小影火 / 监护栏 / 我的”的 ID、标题、顺序和 Fragment 逻辑，视觉使用 T1 深色半透明导航容器。
...
是否使用 `开心.png`：允许作为主状态区品牌视觉使用，建议 48dp 至 96dp；不得作为摄像头、麦克风、位置、状态、SOS 或底部导航图标。

不允许加入的旧样图功能：事件记录、风险分析、支持者管理。

不能修改的业务逻辑：开关回调、位置发送、设备状态发送、SOS 触发、底部导航。

页面完成标准：所有入口可见且可触发；T1 深色背景、半透明信息卡、小影火主状态、每日一句、设备控制、并列入口、SOS 和深色底部导航形成稳定层级；SOS 醒目但不长期大面积高刺激；状态语义色正确；不存在新增功能。
...
以下规则用于约束 6.3—6.8 的视觉一致性：

### 小影火 AI 页面

- 深色沉浸背景，减少大面积浅色空白。
- 小影火作为核心视觉，主按钮使用 `#5B5CEB`。
- `tv_ai_status` 和离线 / 错误状态必须清晰可读。
- 不改变 `btn_start_chat`、`EXTRA_ROLE`、`EXTRA_MODE`、DeepSeek 兜底和 AI 风险逻辑。

### 患者监护栏

- 继承深色背景和半透明卡片。
- 四个入口保持 2x2：消息、位置、通话、画面，不得合并。
- 最近消息使用独立半透明信息卡。
- 保留“监护栏”概念和现有业务文案，不改成“支持者”。

### 患者“我的”

- 深色背景，账户、提示设置、AI 设置、关于分组为半透明卡片。
- 开关在深色表面上必须清晰。
- 退出登录可使用红色描边或低饱和危险样式，不得长期大面积高刺激红色。
- 患者和监护人角色下的显示差异保持现状。

### 离线聊天和正常聊天

- 消息页背景使用深色体系。
- 自己消息使用 `#5B5CEB` 紫色气泡，对端消息使用深色半透明气泡。
- 离线横幅使用灰蓝 / 离线语义状态，不能制造高警报感。
- 输入区域清晰，不与底部导航或系统导航混淆。

### 监护人首页

- 继承深靛紫背景和 T1 半透明卡片。
- 顶部状态卡、实时数据卡、患者消息卡和操作入口保持清晰层级。
- 视频区域使用深色哑光容器；真实视频出现时不得覆盖滤镜、渐变、玻璃遮罩或光晕。
- 请求画面、消息、语音、SOS 和底部导航全部保留，不做成监控后台。
...
- 可使用深色或高对比弹窗。
- 红色只集中于危险标识和确认操作。
- 标题、确认说明、取消、确定求助、位置共享和发送逻辑原样保留。

### 音频和视频通话

- 背景继承 T1 深色沉浸体系。
- 麦克风、挂断、摄像头按钮保持原逻辑。
- `iv_video_stream` 和真实视频区域不得增加紫色遮罩、滤镜、玻璃层或持续动画。
- 音频模式继续隐藏视频区；停止流和生命周期逻辑不变。
...
阶段 6.3—6.8 的每轮截图验收都必须增加 T1 视觉一致性检查：深色背景、半透明卡片、浅色文字、底部导航、SOS、状态语义、信息密度和视频无遮挡均需符合本文件 T1 规则。6.2 登录页保持已通过浅色入口，不在 6.3—6.7 中修改；6.8 只评估是否需要少量品牌衔接，不得未经授权重新实施登录页。
